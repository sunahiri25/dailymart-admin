...
function Brands({ swal }) {
    ...
    function deleteBrand(brand) {
        swal.fire({
            ...
            if (result.isConfirmed) {
                const { _id } = brand;
                axios.delete('/api/brands?_id=' + _id)
    ...
    return (
        <Layout>
            <h1>Brands</h1>
            <label>{editedBrand ? `Edit brand ${editedBrand.name}` : "New brand name "}</label>
            <form onSubmit={saveBrand} className="flex gap-1">
                ...
                <select className="mb-0" value={active} onChange={e => setActive(e.target.value)}>
                    <option value="Active">Active</option>
                    <option value="Inactive">Inactive</option>
                </select>
                <button className="btn-primary py-1 " type="submit">Save</button>
            </form>
            <h1 className="mt-2">Number of brands: {brands.length}</h1>
